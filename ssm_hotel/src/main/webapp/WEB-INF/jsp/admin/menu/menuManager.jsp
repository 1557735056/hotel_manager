...
<head>
    ...
    <link rel="stylesheet" href="${pageContext.request.contextPath}/static/layui/lib/layui-v2.5.5/css/layui.css"
          media="all">
    <link rel="stylesheet" href="${pageContext.request.contextPath}/static/layui/lib/font-awesome-4.7.0/css/font-awesome.css"/>
    <%--    添加layui-dtree的样式--%>
    ...
    layui.extend({
        ...
        let tableIns = table.render({
            ...
            cols: [[
                ...
                {field: 'title', minWidth: 180, title: '菜单名称', align: "center"},
                {field: 'href', minWidth: 200, title: '菜单地址', align: "center"},
                {field: 'spread', minWidth: 100, title: '是否展开', align: "center",templet:function (d) {
                        return d.spread == 1 ?  "<font color='red'>是</font>" : "<font color='blue'>否</font>";
                    }},
                {field: 'icon', minWidth: 100, title: '菜单图标', align: "center",templet:function (d) {
                        return "<i class='"+d.icon+"'>";
                    }},
        ...
        let menuSelectTree =  dtree.renderSelect({
            elem:'#menuSelectTree',//绑定ul标签的id属性赋值
            url:'/admin/menu/loadMenuTree', //请求地址
            dataStyle: "layuiStyle",  //使用layui风格的数据格式
            dataFormat: "list",  //配置data的风格为list
            response:{message:"msg",statusCode:0},  //修改response中返回数据的定义
        ...
        table.on('tool(currentTableFilter)', function (obj) {
            ...
            if (obj.event === 'edit') {
                mainIndex = layer.open({
                    ...
                    success: function () {
                        ...
                        //父级菜单回显
                        dtree.dataInit("menuSelectTree",data.pid);//参数1：下拉菜单树的id属性值,参数2：父 节点ID值
                        dtree.selectVal("menuSelectTree");//参数1：下拉菜单树的id属性值 //判断当前选中节点是否是一级菜单，如果是一级菜单，那么该菜单是没有父级菜单，此时应该显 示请选择
                        if(data.pid==0){
                            //刷新菜单树
                            menuSelectTree.reload();
                        }
            ...
            if (obj.event === 'delete') {
                ...
                $.ajax({
                    type:'get',
                    url:"/admin/menu/checkMenuHasChild",
                    data:{"id":data.id},
                    dataType: 'json',
                    success:function(result){
