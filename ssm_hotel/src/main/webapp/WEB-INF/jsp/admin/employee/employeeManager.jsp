    ...
    layui.use(['form', 'table','laydate','jquery'], function () {
        ...
        let tableIns = table.render({
            ...
            cols: [[
                ...
                {field: 'sex', width: 80, title: '性别', align: "center",templet:function (data) {
        ...
        table.on('tool(currentTableFilter)', function (obj) {
            ...
            if (obj.event === 'grantRole') {
                grantRoleWindow(obj.data);
                return false;
            }

        });
        //监听行工具栏事件
        function grantRoleWindow(data)
        {
            mainIndex = layer.open({
                type: 1,
                title: "分配[<font color='red'>"+data.name+"</font>]角色窗口",
                area: ["400px", "550px"],
                ...
                yes: function(index, layero){
                    //获取选中行
                    let params = table.checkStatus("roleTable");
                    //判断是否有选中行
                    if(params.data.length>0){
                        //发送Ajax请求保存选中行的角色id和员工id
                        let idArr = [];
                        //循环获取选中的值
                        for (let i = 0; i < params.data.length; i++) {
                            idArr.push(params.data[i].id);
                        }
                        //将字符串转换成字符串 ids 选中的菜单id值
                        let ids = idArr.join(",");
                        //发送ajx请求
                        $.post("/admin/employee/saveEmployeeRole",{"ids":ids,"roleId":data.id},function (result) {
                            layer.msg(result.message);
                        })
                    }else{
                        //请选择要分配的菜单
        ...
        function initTableData(data)
        {
            table.render({
                elem: '#roleTable'
                ,url: '${pageContext.request.contextPath}/admin/role/initRoleListByEmpId/?id='+data.id //数据接口
                ,cols: [[ //表头
                    {type:"checkbox",width:50},
                    {field: 'id', minWidth:100,align:'center',title:'角色编号'}
                    ,{field: 'rolename', title: '角色名称', minWidth:120,align:'center'}
                    ,{field: 'roledesc', title: '角色描述', minWidth:120,align:'center'}
